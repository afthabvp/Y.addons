...
import com.accenture.jalo.CSRCustomerDetails;
import com.accenture.jalo.CollectOrder;
import com.accenture.jalo.Config;
import de.hybris.platform.cms2lib.components.UCOIDOnOrderConfirmationComponent;
...
/**
 * Generated class for type <code>QrcodeaddonManager</code>.
 */
@SuppressWarnings({"deprecation","unused","cast","PMD"})
public abstract class GeneratedQrcodeaddonManager extends Extension
{
	...
	public CollectOrder createCollectOrder(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( QrcodeaddonConstants.TC.COLLECTORDER );
	...
	public Config createConfig(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( QrcodeaddonConstants.TC.CONFIG );
	...
	public CSRCustomerDetails createCSRCustomerDetails(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( QrcodeaddonConstants.TC.CSRCUSTOMERDETAILS );
	...
	public UCOIDOnOrderConfirmationComponent createUCOIDOnOrderConfirmationComponent(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( QrcodeaddonConstants.TC.UCOIDONORDERCONFIRMATIONCOMPONENT );
			return (UCOIDOnOrderConfirmationComponent)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating UCOIDOnOrderConfirmationComponent : "+e.getMessage(), 0 );
		}
	}
	
	public UCOIDOnOrderConfirmationComponent createUCOIDOnOrderConfirmationComponent(final Map attributeValues)
	{
		return createUCOIDOnOrderConfirmationComponent( getSession().getSessionContext(), attributeValues );
	}
	
	@Override
	public String getName()
	{
		return QrcodeaddonConstants.EXTENSIONNAME;
	}
	...
	{
		Collection<Product> coll = (Collection<Product>)item.getProperty( ctx, QrcodeaddonConstants.Attributes.User.RECENTLYVIEWEDPRODUCTS);
	...
	{
		item.setProperty(ctx, QrcodeaddonConstants.Attributes.User.RECENTLYVIEWEDPRODUCTS,value == null || !value.isEmpty() ? value : null );
	}
	...
	{
		return (String)item.getProperty( ctx, QrcodeaddonConstants.Attributes.Order.UCOID);
	}
	...
	{
		item.setProperty(ctx, QrcodeaddonConstants.Attributes.Order.UCOID,value);
	}
	...
	{
		return (String)item.getProperty( ctx, QrcodeaddonConstants.Attributes.Customer.UUID);
	}
	...
	{
		item.setProperty(ctx, QrcodeaddonConstants.Attributes.Customer.UUID,value);
	}
